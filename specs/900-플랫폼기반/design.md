...
내부에서는 성공·실패, 시각, IP, User-Agent를 로그인 이력에 기록한다. 연속 실패 횟수와 잠금 시간은 환경 설정으로 주입하고 코드 상수로 숨기지 않는다. 확정 기본값은 연속 5회 실패 시 15분 잠금이다.

### 3.3 세션

JWT 대신 서버 저장형 불투명 세션을 사용한다. 무작위 토큰 원문은 `Secure`, `HttpOnly`, `SameSite=Lax` 쿠키로만 전달하고 DB에는 SHA-256 다이제스트만 저장한다. 세션 최대 수명은 8시간, 유휴 만료는 30분이며 환경 설정으로 주입한다. 이 방식은 전체 로그아웃, 퇴사 차단, 권한 변경의 다음 요청 반영을 별도 블랙리스트 없이 지원한다.
...
010 연결 계약은 이미 있는 포트를 실제 어댑터로 채운다. 새 이름을 만들어 010을 다시 짜지 않는다.

- `CurrentActorProvider`: 세션의 계정·사원 ID
- `AuthorizationChecker`: 등록·기록카드·재직 쓰기/읽기 권한
- `OrganizationReader`: 등록·발령 시점의 사업장·부서 유효성
- `OrganizationSnapshotReader`: 기준일 조직도·정원
- `AccountAccessScheduler`: 퇴사일 다음 날 계정 비활성

권한 또는 조직 조회 실패는 fail-closed로 거부한다.
...
- IP와 User-Agent는 로그인 이력 목적에 필요한 범위로 제한한다. 로그인 이력과 권한 변경 이력의 보존 기간은 3년이다. 3년 만료 자동 파기는 이번 범위가 아니다.
...
PR #18 사용자 확정. 환경 설정으로 주입하고 코드 상수로 숨기지 않는다.

- 비밀번호: 최소 12자, 최근 5개 재사용 금지. 문자 종류(대소문자·숫자·특수문자) 조합은 이번 확정에 없어 강제하지 않는다.
- 로그인 잠금: 연속 5회 실패 시 15분
- 세션: 최대 8시간, 유휴 30분
- 로그인·권한 변경 이력: 3년 보존. 만료 자동 파기는 후속

2단계 인증과 비밀번호 재설정은 여전히 후속 범위다.
